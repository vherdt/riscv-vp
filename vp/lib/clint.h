...
struct CLINT : public clint_if,
    ...
    //
    // core local interrupt controller (provides local timer interrupts with memory mapped configuration)
    //
    // send out interrupt if: *mtime >= mtimecmp* and *mtimecmp != 0*
    //
    // *mtime* is a read-only 64 bit timer register shared by all CPUs (accessed by MMIO and also mapped into the CSR address space of each CPU)
    //
    // for every CPU a *mtimecmp* register (read/write 64 bit) is available
    //
    //
    // Note: the software is responsible to ensure that no overflow occurs when writing *mtimecmp* (see RISC-V privileged ISA spec.):
    //
    ...
    static constexpr uint64_t scaler = 5000000;    // scale from PS resolution (default in SystemC) to US resolution (apparently required by FreeRTOS)
    ...
    void run() {
        while (true) {
            ...
            //std::cout << "[clint] process mtimecmp=" << mtimecmp << ", mtime=" << mtime << std::endl;
            ...
            if (mtimecmp > 0 && mtime >= mtimecmp) {
                ...
                if (mtimecmp > 0) {
                    ...
                    //std::cout << "[clint] re-schedule event at trigger loop " << goal - time << std::endl;
