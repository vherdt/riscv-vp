...
struct AESSboxes : public sc_core::sc_module {
	simple_target_socket<AESSboxes> tsock;

	unsigned int adress_size = sizeof(aes_table) * 2;

	AESSboxes(sc_module_name) {
		tsock.register_b_transport(this, &AESSboxes::transport);
	}

	~AESSboxes() {}

	void read_data(unsigned addr, uint8_t *dst, unsigned num_bytes) {
		sc_assert(num_bytes == 1 || num_bytes == 4);
		const aes_table *table = addr < sizeof(aes_table) ? &_aes_encrypt_table : &_aes_decrypt_table;
		intptr_t offset = addr < sizeof(aes_table) ? 0 : sizeof(aes_table);
		addr = addr - offset;
		if(addr < sizeof(aes_table::sbox))
		{
			sc_assert(num_bytes == 1);
			memcpy(dst, table->sbox, 1);
		}
		else if(addr < sizeof(aes_table::sbox) + sizeof(aes_table::table))
		{
			sc_assert(num_bytes == 4);
			memcpy(dst, table->sbox, 4);
		}
		else
		{
			sc_assert(false && "illegal memory addr");
		}
	}
