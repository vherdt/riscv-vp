...
Options parse_command_line_arguments(int argc, char **argv) {
	...
	try {
		...
		desc.add_options()("help", "produce help message")("memory-start", po::value<unsigned int>(&opt.mem_start_addr),
		    ...
		    "max-instrs", po::value<unsigned int>(&opt.max_test_instrs),
		    "maximum number of instructions to execute (soft limit, checked periodically)")(
		    ...
		    "isa", po::value<std::string>(&opt.isa)->default_value("imacfnus"),
		    "output filename for the test execution signature")(
