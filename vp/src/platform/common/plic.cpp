...
int PLIC::hart_0_get_next_pending_interrupt(bool consider_threshold) {
	...
	// std::cout << "[vp::plic] get_next_pending_interrupt(consider_threshold="
	// << consider_threshold << ")" << std::endl; std::cout << "[vp::plic]
	// pending interrupts 1: " << pending_interrupts_1 << std::endl; std::cout
	// << "[vp::plic] pending interrupts 2: " << pending_interrupts_2 <<
	// std::endl; std::cout << "[vp::plic] priority threshold: " <<
	// hart_0_priority_threshold << std::endl;
...
void PLIC::register_access_callback(const vp::map::register_access_t &r) {
	...
	if (r.write && r.vptr == &hart_0_claim_response) {
		...
		if (hart_0_has_pending_enabled_interrupts()) {
			assert(hart_0_eip);
			target_hart->trigger_external_interrupt();  // trigger again to make
