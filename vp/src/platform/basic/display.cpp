...
void Display::createSM() {
	...
	if ((shmid = shmget(SHMKEY, sizeof(Framebuffer), IPC_CREAT | 0666)) < 0) {
		std::cerr << "Could not get " << sizeof(Framebuffer)
		          << " Byte of shared Memory " << int(SHMKEY) << " for display"
		          << std::endl;
		perror(NULL);
		exit(1);
	}
...
void Display::transport(tlm::tlm_generic_payload &trans,
	...
	assert((addr + len <= sizeof(Framebuffer)) &&
	       "Access display out of bounds");

	if (cmd == tlm::TLM_WRITE_COMMAND) {
		if (addr == offsetof(Framebuffer, command) &&
		    len == sizeof(Framebuffer::Command)) {  // apply command
			switch (*reinterpret_cast<Framebuffer::Command *>(ptr)) {
				...
				case Framebuffer::Command::fillFrame:
					fillFrame(frame.buf->parameter.fill.frame,
					          frame.buf->parameter.fill.color);
					break;
				case Framebuffer::Command::applyFrame:
					frame.buf->activeFrame++;
					memcpy(&frame.buf->getInactiveFrame(),
					       &frame.buf->getActiveFrame(), sizeof(Frame));
					break;
			...
			// reset parameter
			memset(reinterpret_cast<void *>(&frame.buf->parameter), 0,
			       sizeof(Framebuffer::Parameter));
		} else if (addr >= offsetof(Framebuffer, parameter) &&
		           addr + len <
		               offsetof(Framebuffer, parameter) +
		                   sizeof(Framebuffer::Parameter)) {  // write the
			                                                  // parameter
			...
			memcpy(&frame.raw[addr], ptr, len);
	           ...
	           addr < sizeof(Framebuffer)) {  // read whatever you like
...
void Display::fillFrame(Framebuffer::Type type, Color color) {
	...
	uint64_t *rawFrame =
	    reinterpret_cast<uint64_t *>(&frame.buf->getFrame(type).raw);
...
void Display::drawLine(Framebuffer::Type type, PointF from, PointF to,
                       Color color) {
