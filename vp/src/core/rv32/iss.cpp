...
Opcode::Mapping ISS::exec_step() {
  ...
  if (debug) {
    ...
    switch (Opcode::getType(op)) {
      case Opcode::Type::R:
        ...
        break;
      case Opcode::Type::I:
        printf(COLORFRMT ", " COLORFRMT ", 0x%x", regcolors[instr.rd()], regnames[instr.rd()], regcolors[instr.rs1()],
               regnames[instr.rs1()], instr.I_imm());
        break;
      case Opcode::Type::S:
        printf(COLORFRMT ", " COLORFRMT ", 0x%x", regcolors[instr.rs1()], regnames[instr.rs1()], regcolors[instr.rs2()],
               regnames[instr.rs2()], instr.S_imm());
        break;
      case Opcode::Type::B:
        printf(COLORFRMT ", " COLORFRMT ", 0x%x", regcolors[instr.rs1()], regnames[instr.rs1()], regcolors[instr.rs2()],
               regnames[instr.rs2()], instr.B_imm());
        break;
      case Opcode::Type::U:
  ...
  switch (op) {
    ...
    case Opcode::ECALL: {
      ...
      int ans =
          sys->execute_syscall((uint32_t)regs[RegFile::a7], (uint32_t)regs[RegFile::a0], (uint32_t)regs[RegFile::a1],
                               (uint32_t)regs[RegFile::a2], (uint32_t)regs[RegFile::a3]);
      regs[RegFile::a0] = ans;
    } break;
    ...
    case Opcode::LR_W: {
      // TODO: in multi-threaded system (or even if other components can access the memory independently, e.g. through
      // DMA) need to mark this addr as reserved
  ...
  // NOTE: writes to zero register are supposedly allowed but must be ignored (reset it after every instruction, instead
  // of checking *rd != zero* before every register write)
...
void ISS::return_from_trap_handler() {
  // std::cout << "[vp::iss] return from trap handler @time " << quantum_keeper.get_current_time() << " to pc " <<
  // std::hex << csrs.mepc->reg << std::endl;

  // NOTE: assumes a SW based solution to store/re-store the execution context, since this appears to be the RISC-V
  // convention
  pc = csrs.mepc->reg;
...
void ISS::switch_to_trap_handler() {
  ...
  // std::cout << "[vp::iss] switch to trap handler @time " << quantum_keeper.get_current_time() << " @last_pc " <<
  // std::hex << last_pc << " @pc " << pc << std::endl;
  ...
  // for SW traps the address of the instruction causing the trap/interrupt (i.e. last_pc, the address of the
  // ECALL,EBREAK - better set pc=last_pc before taking trap) for interrupts the address of the next instruction to
...
void ISS::run_step() {
  ...
  // Do not use a check *pc == last_pc* here. The reason is that due to interrupts *pc* can be set to *last_pc*
  // accidentally (when jumping back to *mepc*).
