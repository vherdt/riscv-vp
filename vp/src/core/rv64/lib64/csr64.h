...
struct csr_base {
    ...
    virtual int_rv_t unchecked_read() = 0; //xlen values (rv64)
    virtual void unchecked_write(int_rv_t val) = 0;

    virtual void init() {}

    int_rv_t read(PrivilegeLevel access_level=PrivilegeLevel::Machine) {
        ensure (level <= access_level);
        unchecked_read();
    }

    void write(int_rv_t val, PrivilegeLevel access_level=PrivilegeLevel::Machine) {
        ensure (level <= access_level);
        ensure (mode & Write);
        unchecked_write(val);
    }

    void clear_bits(int_rv_t mask, PrivilegeLevel access_level=PrivilegeLevel::Machine) {
        write(unchecked_read() & ~mask, access_level);
    }

    void set_bits(int_rv_t mask, PrivilegeLevel access_level=PrivilegeLevel::Machine) {
        write(unchecked_read() | mask, access_level);
    }

    csr_base(uint_rv_t addr, const char *name)//TODO 12bit
            : addr(addr), name(name) {
...
#define INCLUDE_CSR_MIXIN										\
	using csr_base::csr_base;									\
																\
	virtual int_rv_t unchecked_read() override {					\
		return reg;												\
	}															\
																\
	virtual void unchecked_write(int_rv_t val) override {		\
		reg = val;												\
	}



struct csr_64_new : public csr_base {//some CSRs are always 64bit even for rv32 and rv128 //TODO refactor names
    INCLUDE_CSR_MIXIN;

    int64_t reg = 0;
};

struct csr_XLEN : public csr_base {
    INCLUDE_CSR_MIXIN;
...
struct csr_64 { //TODO unnecessary
    union {
        int64_t reg = 0;
        struct {
            int32_t low;
...
enum csr_addresses {
    ...
    //CSR_CYCLEH_ADDR = 0xC80, Instructions accessing the high part of CSR should raise exceptions under rv64
    CSR_TIME_ADDR  = 0xC01,
...
struct csr_table { //TODO rv64

/*#define CSR_TABLE_DEFINE_COUNTER(basename)      \
    csr_64_new *basename ## _root = 0;              \
    ...
    std::map<int_rv_t, csr_base*> addr_to_csr; //TODO 32bit?

    csr_base &at(int_rv_t addr) {
...
/*#define CSR_TABLE_ADD_COUNTER_64(basename, addr)                                            \
    ...
    void setup() {
        ...
        cycle = _register(new csr_64_new(0xC00, "cycle"));
        time = _register(new csr_64_new(0xC00, "time"));
        instret = _register(new csr_64_new(0xC00, "instret"));
        /* machine csrs */
        mvendorid = _register(new csr_mvendorid(0xF11, "mvendorid"));
        marchid = _register(new csr_XLEN(0xF12, "marchid"));
        mimpid = _register(new csr_XLEN(0xF13, "mimpid"));
        mhartid = _register(new csr_XLEN(0xF14, "mhartid"));
        ...
        mscratch = _register(new csr_XLEN(0x340, "mscratch"));
        mepc = _register(new csr_mepc(0x341, "mepc"));
        mcause = _register(new csr_mcause(0x342, "mcause"));
        mtval = _register(new csr_XLEN(0x343, "mtval"));
