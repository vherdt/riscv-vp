...
typedef uint_rv_t Elf32_Off;	// Unsigned offset //these differ between ELFs (xlen)
typedef uint_rv_t Elf32_Addr;	// Unsigned address
...
struct ELFLoader {
...
#if DEBUGPRINT_INSTRUCTIONS
        printf("\n|ELF HEADER|\n");
        printf("e_type:%d\n",(uint32_t)hdr->e_type);
        printf("e_machine:%d\n",(uint32_t)hdr->e_machine);
        printf("e_entry:%d\n",(uint32_t)hdr->e_entry);
        printf("e_phoff:%d\n",(uint32_t)hdr->e_phoff);
        printf("e_shoff:%d\n",(uint32_t)hdr->e_shoff);
        printf("e_flags:%d\n",(uint32_t)hdr->e_flags);
#endif
    }
    ...
    uint_rv_t get_memory_end() { //TODO rv64
