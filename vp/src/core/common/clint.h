...
struct CLINT : public clint_if, public sc_core::sc_module {
  ...
  //
  // *mtime* is a read-only 64 bit timer register shared by all CPUs (accessed by MMIO and also mapped into the CSR
  // address space of each CPU)
  //
  // for every CPU a *mtimecmp* register (read/write 64 bit) is available
  //
  //
  // Note: the software is responsible to ensure that no overflow occurs when writing *mtimecmp* (see RISC-V privileged
  // ISA spec.):
  //
  ...
  static constexpr uint64_t scaler =
      2000000;  // scale from PS resolution (default in SystemC) to US resolution (apparently required by FreeRTOS)
