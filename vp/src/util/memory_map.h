...
struct RegisterRange {
    ...
    RegisterRange(uint64_t start, uint64_t size)
            : start(start) {
        assert (size > 0);
    ...
    void write(uint64_t addr, const uint8_t *src, size_t len, tlm::tlm_generic_payload &trans, sc_core::sc_time &delay) {
...
struct ArrayView {
    ...
    ArrayView(RegisterRange &r)
            : regs(r), size(r.mem.size() / sizeof(T))
    {
    ...
    T *end() { return &ptr[size-1]; }
    const T *end() const { return &ptr[size-1]; }
};


namespace vp { namespace mm {
