...
void drawBackground()
{
	...
	{
		display::drawLine(Framebuffer::Type::background, Point(0, i), Point(screenWidth, i), fromRGB(i & 0x7, 0, i >> 3));
		if(i % 5 == 0)
			display::applyFrame();
	}
}
...
{
	PointF progress = horizontal ?
							PointF(extent.x * completion, extent.y) :
